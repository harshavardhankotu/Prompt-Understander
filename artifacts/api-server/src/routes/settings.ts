...
async function getOrCreateSettings(userId: string, role: string) {
  ...
  const [created] = await db.insert(userSettingsTable).values({ userId, settings: defaults }).returning();
  return created;
}
...
router.put("/settings/my", requireAuth, async (req, res): Promise<void> => {
  ...
  const merged = { ...(existing.settings as object), ...body.data };
  ...
  res.json({ settings: updated.settings, updatedAt: updated.updatedAt.toISOString() });
});
