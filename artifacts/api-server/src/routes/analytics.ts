...
router.get("/analytics/admin", requireAuth, async (req, res): Promise<void> => {
  ...
  res.json({
    ...
    sectorActivity: sectorActivity.map(s => ({ categoryName: s.categoryName, categorySlug: s.categorySlug, requirementCount: Number(s.requirementCount), priceFloor: Number(s.priceFloor ?? 0) })),
